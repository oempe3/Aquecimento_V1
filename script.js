                ...
                tooltip: {
                    ...
                    callbacks: {
                        ...
                        label: function(context) {
                            ...
                            const motorName = context.dataset.label;
                            const temp = dataPoint.y.toFixed(1);
                            const timeFromStart = timeToMinutes(dataPoint.x).toFixed(1);
                            const phase = dataPoint.phase === 'heating' ? '2 estágios ligados' : '1 estágio ligado';
                            const rate = Math.abs(dataPoint.rate).toFixed(2);
                            ...
                            if (dataPoint.phase === 'heating') {
                                tooltip.push(`Taxa de elevação: ${rate}°C/min`);
                            }
...
function updateInfoCards(data) {
    ...
    motors.forEach((motorKey, index) => {
        ...
        document.getElementById(`motor${motorNum}-1stage`).textContent = consumption.avg1Stage.toFixed(2);
        document.getElementById(`motor${motorNum}-2stage`).textContent = consumption.avg2Stage.toFixed(2);
        document.getElementById(`motor${motorNum}-total`).textContent = consumption.avgTotal.toFixed(2);
    });
}
